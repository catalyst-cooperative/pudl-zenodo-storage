...
from pudl_zenodo_storage.frictionless import ferc6


class TestFerc6Source:
    ...
    def test_single_file_resource(self, zenodo_url):
        ...
        package = ferc6.datapackager([fake_resource])
        res = package["resources"][0]

        assert Package(descriptor=package).valid
        assert res["name"] == name
        assert res["title"] == f"ferc6-{year}"
        assert res["path"] == url
